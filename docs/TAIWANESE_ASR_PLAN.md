...
2. **前端文案 =「台語（翻譯成中文）」**（i18n key `transcription.langTaiwanese`；en：Taiwanese (translated to Chinese)；2026-07-06 從「輸出為漢字」改字）。
   > 用詞備註：模型實際行為是把台語發音轉寫成對應漢字（類似粵語可用漢字書寫但唸法不同），不是把語意轉換成標準中文書面語的「翻譯」。目前文案選用「翻譯成中文」是產品決策，非技術描述——如使用者回饋覺得誤導（預期得到中文書面語卻拿到台語漢字逐字稿），可考慮回退成更準確但較長的說法。
...
在 staging worker 的 `.env.worker` 加 `WHISPER_MODEL_NAN_TW=/home/ec2-user/breeze-test/breeze-asr-26-ct2`，從前端選「台語（翻譯成中文）」上傳真實台語音檔跑一個任務，確認：
