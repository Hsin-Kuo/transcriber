...
> **儲存層套件 `src/utils/storage/`**：`backend`（local / S3 切換的 deep 核心——`is_aws`、S3 client、`validate_task_id`、`detect_content_type`、`parse_s3_key`、`validate_aws_config`），`compact`（[[Compact audio]] 讀寫，tier 路徑分區住這），`handoff`（[[Handoff audio]] upload/download/delete/sweep）。兩個 store 都建在 `backend` 上、互不依賴。**不要重建一個 re-export facade**（呼叫端直接 import 對應 submodule）。原 `storage_service.py` god module 已拆除，死碼 output 群組已刪。
