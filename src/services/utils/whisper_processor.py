...
# Hybrid 覆蓋補轉：batched 是「單一溫度、無 fallback」，困難音段（換講者起音/低音量）會
# 整段解爆被丟掉；sequential 的 generate_with_fallback 會升溫重試救回。開啟後 → batched 全轉
# 完，對「未覆蓋且夠長的空洞」用 sequential 補轉再合併 → 拿回 batched 速度 + 補回掉的真實
# 語音（成本 ∝ 掉段比例）。預設 false。僅 GPU+batched 路徑有意義（sequential 本就有 fallback）。
_HYBRID_RESCUE = os.getenv("WHISPER_HYBRID_RESCUE", "false").strip().lower() in ("true", "1", "yes")
HYBRID_GAP_MIN_SEC = float(os.getenv("WHISPER_HYBRID_GAP_MIN_SEC", "2.0"))  # 空洞 ≥ 此長度才值得補轉
...
def _find_uncovered_gaps(
    segments: List[Dict], total_duration: float, min_gap: float
) -> List[Tuple[float, float]]:
    """找出 segments 未覆蓋、且長度 ≥ min_gap 的時間空洞 [(start, end), ...]。

    用於 hybrid 補轉:batched 可能把困難音段整段解爆丟掉，這些空洞就是「該補轉的區間」。
    含頭尾空洞（首段前、末段後到 total_duration）。重疊 segments 會先被吸收。
    """
    if total_duration <= 0:
        return []
    covered = sorted(
...
def _merge_segments_by_start(base: List[Dict], extra: List[Dict]) -> List[Dict]:
    """把 extra（補轉段）併進 base（batched 段），依 start 排序。

    extra 段由設計落在 base 的空洞內 → 不與 base 重疊；依 start 排序即得正確時序，
    下游 full_text 串接 / align / diarization 都依賴這個時序。
    """
    merged = list(base) + list(extra)
    merged.sort(key=lambda s: (float(s.get("start", 0.0)), float(s.get("end", 0.0))))
    return merged
...
class WhisperProcessor:
    ...
    def _transcribe_with_timestamps(
        ...
        used_batched = self._has_gpu() and _USE_BATCHED
        if used_batched:
            ...
            # 掉段問題另可由 WHISPER_HYBRID_RESCUE=true 補回（見下方 _hybrid_rescue）。
    ...
    def _hybrid_rescue(
        ...
        """batched 輸出後，對未覆蓋的空洞用 sequential（含溫度 fallback）補轉再合併。

        batched 是單一溫度、無 fallback：困難音段（換講者起音/低音量）解爆 → 整段掉。
        sequential 的 `generate_with_fallback` 會升溫重試把它救回。這裡只對「空洞」補轉
        （clip_timestamps 指定區間時 VAD 自動忽略），成本 ∝ 掉段比例（典型 ~3-4%）。

        回傳合併後的 segments（含補轉段的 words，供後續 _resegment 使用）。對下游透明：
        full_text 串接 / align / diarization 仍消費同一份 segments list。
        """
        gaps = _find_uncovered_gaps(segments_list, total_duration, HYBRID_GAP_MIN_SEC)
        if not gaps:
            return segments_list

        # sequential clip_timestamps 格式：逗號分隔的秒數 "s1,e1,s2,e2,..."
        clip = ",".join(f"{s:.2f},{e:.2f}" for s, e in gaps)
        # clip_timestamps 指定時 VAD 會被忽略 → 移除 VAD 參數、明確關閉；其餘沿用（含溫度 fallback 預設）
        kw = {k: v for k, v in transcribe_kwargs.items() if k not in ("vad_filter", "vad_parameters")}
        try:
            rescue_segments, _info = self.model.transcribe(
                str(audio_path), clip_timestamps=clip, vad_filter=False, **kw
            )
            rescued = [
            ...
            # 補轉失敗不該打斷主轉錄 → 回原 batched 結果
            log.warning("whisper.hybrid_rescue.failed", error=str(e))
            return segments_list

        gap_total = sum(e - s for s, e in gaps)
        rescued_total = sum(r["end"] - r["start"] for r in rescued)
        log.info(
            "whisper.hybrid_rescue",
            gaps=len(gaps),
            gap_seconds=round(gap_total, 1),
            rescued_segments=len(rescued),
            rescued_seconds=round(rescued_total, 1),
        )
        return _merge_segments_by_start(segments_list, rescued)
